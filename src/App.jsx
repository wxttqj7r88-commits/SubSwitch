...
// ============================================================
// 追いLINE 3問（Q1→Q2追い込みテイスト→Q3副業目標）
// ============================================================
const oiQuestions = [
...
const buildOiMessage = (ans) =>
`【追いLINE登録希望】\n取り組んでいること：${ans.current || ""}\n追い込みスタイル：${ans.style || ""}\n目標：${ans.goal || ""}`;

// ============================================================
// バラツキチェッカー用プロンプト
// ============================================================
const buildBaraPrompt = (q1, q2) =>
`以下は副業に取り組んでいる美容師の状況です。\n\n【副業の収益化のために取り組んでいること】\n${q1}\n\n【現在の優先順位・力の入れ具合】\n${q2}\n\n以下の形式で分析してください：\n\n## 集中度スコア\n100点満点で数値のみ（例：42点）と一言コメント\n\n## バラツキの原因\n箇条書きで2〜3個\n\n## 今すぐ一点集中すべきこと\n1つだけ、理由も添えて\n\n## やめていいこと\n1〜2個、理由も添えて\n\n中学生でも分かる言葉で、背中を押すような文体で書いてください。`;
...
- { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .ss-card { margin: 10px 14px 0; background: #141414; border: 1px solid #222; border-radius: 14px; padding: 18px 16px 40px; cursor: pointer; transition: border-color .2s, transform .15s; position: relative; overflow: hidden; }
  ...
  .ss-card-tag { font-size: 10px; padding: 2px 9px; border-radius: 20px; border: 1px solid #c9a84c55; color: #c9a84c; white-space: nowrap; margin-top: 2px; }
  .ss-card-desc { font-size: 12px; color: #666; line-height: 1.7; margin-top: 6px; white-space: pre-line; }
  .ss-card-arrow { position: absolute; right: 16px; bottom: 16px; color: #2e2e2e; font-size: 14px; }
  .ss-panel { margin: 14px 14px 0; background: #141414; border: 1px solid #c9a84c33; border-radius: 16px; padding: 22px 18px; animation: ssUp .22s ease; }
  ...
  .ss-panel-title { font-size: 17px; font-weight: 700; color: #fff; margin-bottom: 8px; line-height: 1.4; }
  ...
  .ss-btn-ghost { background: transparent; color: #666; border: 1px solid #2a2a2a !important; }
  .ss-btn-ghost:active { border-color: #555 !important; color: #aaa; }
  .ss-btn-line { background: #06b96a; color: #fff; display: flex; align-items: center; justify-content: center; gap: 8px; }
  .ss-btn-line:active { background: #05a85e; transform: scale(.98); }
  .ss-input { width: 100%; background: #1a1a1a; border: 1px solid #2a2a2a; border-radius: 10px; color: #f0ede8; font-family: inherit; font-size: 14px; line-height: 1.7; padding: 12px 14px; resize: vertical; outline: none; margin-top: 8px; transition: border-color .2s; }
  ...
  .ss-hint { font-size: 11px; color: #444; margin-top: 4px; line-height: 1.6; }
  .ss-progress { height: 3px; background: #1e1e1e; border-radius: 2px; margin-bottom: 20px; overflow: hidden; }
  .ss-progress-fill { height: 100%; background: linear-gradient(90deg,#c9a84c,#e8d48a); border-radius: 2px; transition: width .4s; }
  ...
  .ss-point-btn { background: transparent; border: 1px solid rgba(201,168,76,.3); border-radius: 7px; color: #c9a84c; font-size: 11px; padding: 5px 12px; cursor: pointer; display: flex; align-items: center; gap: 5px; font-family: inherit; margin-top: 12px; }
  .ss-point-box { margin-top: 8px; background: rgba(201,168,76,.06); border: 1px solid rgba(201,168,76,.18); border-radius: 9px; padding: 12px 14px; font-size: 12px; color: #c9a84c; line-height: 1.8; animation: ssUp .2s ease; }
  .ss-score { text-align: center; padding: 16px 0 8px; }
  .ss-score-num { font-size: 60px; font-weight: 700; line-height: 1; }
  .ss-score-label { font-size: 12px; color: #666; margin-top: 4px; }
  .ss-result-box { background: #111; border: 1px solid #222; border-radius: 10px; padding: 14px 16px; margin-bottom: 10px; }
  .ss-result-box h3 { font-size: 12px; color: #c9a84c; margin-bottom: 8px; font-weight: 600; }
  .ss-result-box p { font-size: 13px; color: #bbb; line-height: 1.8; white-space: pre-wrap; }
  .ss-loading { text-align: center; padding: 48px 0; }
  .ss-spinner { width: 36px; height: 36px; border: 3px solid #222; border-top-color: #c9a84c; border-radius: 50%; animation: spin .8s linear infinite; margin: 0 auto 16px; }
  @keyframes spin { to { transform: rotate(360deg); } }
  .ss-loading p { font-size: 13px; color: #555; }
  ...
  .ss-sub-card:active { border-color: #c9a84c33; transform: scale(.985); }
  ...
  .ss-nisa-row { display: flex; gap: 10px; margin-top: 8px; }
  .ss-nisa-row .ss-input { flex: 2; margin-top: 0; }
  .ss-nisa-row .ss-input-sm { flex: 1; margin-top: 0; }
  .ss-nisa-add { background: #1e1e1e; border: 1px dashed #333; border-radius: 10px; padding: 10px; text-align: center; font-size: 13px; color: #555; cursor: pointer; margin-top: 8px; transition: all .18s; }
  .ss-nisa-add:active { border-color: #c9a84c55; color: #c9a84c; }
  .ss-diag-result { white-space: pre-wrap; font-size: 13px; color: #ccc; line-height: 1.9; background: #0f0f0f; border: 1px solid #1e1e1e; border-radius: 12px; padding: 18px; margin-bottom: 14px; max-height: 400px; overflow-y: auto; }
  `;

// ============================================================
// メインアプリ
// ============================================================
export default function SubSwitch() {
...
```
  ...
  {phase === "q" && (
    <div className="ss-panel">
      ...
      <div style={{ fontSize: 11, color: "#444", lineHeight: 1.6 }}>💡 {cq.hint}</div>
      {cq.type === "textarea"
      ...
      {answers[cq.id]?.trim() && (
        <>
          ...
          {showPoint && <div className="ss-point-box"><span style={{ opacity:.5, fontSize:10, display:"block", marginBottom:4 }}>📌 このポイントを参考に見直そう</span>{cq.point}</div>}
        </>
      )}
      <div style={{ display: "flex", gap: 10, marginTop: 20 }}>
        ...
        <button className="ss-btn ss-btn-gold" style={{ flex:1 }} onClick={next}>
          {activeQ === total - 1 ? "完了して確認する ✓" : "次へ →"}
        </button>
      </div>
  ...
  {phase === "result" && (
    <div className="ss-panel">
      <div style={{ textAlign:"center", marginBottom:16 }}>
        ...
        <div style={{ fontSize:12, color:"#666", marginTop:4 }}>プロンプトを生成しました。<br />Claude か ChatGPT に貼り付けて送信してください。</div>
      </div>
      ...
      <div style={{ textAlign:"center", fontSize:12, color:"#444", lineHeight:1.7, marginBottom:14 }}>
        コピー後、<strong style={{ color:"#777" }}>Claude</strong> か <strong style={{ color:"#777" }}>ChatGPT</strong> を開いて<br />貼り付けて送信してください 🚀
      </div>
...
<button className=“ss-btn ss-btn-gold” style={{ flex:1 }} onClick={next} disabled={!answers[cq.id]?.trim()}>
{step === oiQuestions.length - 1 ? “確認する ✓” : “次へ →”}
</button>
</div>
</>
)}
...
<button className=“ss-btn ss-btn-line” onClick={() => openLine(msg)}><LineIcon /> LINEで送信する</button>
...
<button className=“ss-btn ss-btn-line” style={{ marginTop:18 }} onClick={() => openLine(msg)}><LineIcon /> LINEで送信する</button>
...
{rows.length > 1 && (
<button onClick={() => removeRow(i)} style={{ background:“none”, border:“none”, color:”#555”, fontSize:18, cursor:“pointer”, padding:“0 4px” }}>×</button>
)}
</div>
))}
<div className="ss-nisa-add" onClick={addRow}>＋ 銘柄を追加する</div>
<button className=“ss-btn ss-btn-line” style={{ marginTop:18 }} onClick={() => openLine(msg)} disabled={validRows.length === 0}><LineIcon /> LINEで送信する</button>
<button className="ss-btn ss-btn-ghost" onClick={back}>← 戻る</button>
</div>
</div>
);
...
<path d="M20 7C12.268 7 6 12.477 6 19.2c0 4.07 2.163 7.677 5.54 10.01-.243.882-.784 3.29-.898 3.8-.143.63.231.623.487.453.2-.134 3.19-2.107 4.48-2.963A16.4 16.4 0 0020 31.4c7.732 0 14-5.477 14-12.2S27.732 7 20 7z" fill="white"/>
</svg>
);
}
